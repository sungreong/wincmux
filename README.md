...
- Main-to-renderer stream IPC now fast-flushes small output for the focused active session with `setImmediate`, shaving the normal batch timer from interactive echo/prompt updates while keeping large/background output batched.
- Core session output now fast-flushes small PTY output shortly after `session.write`, reducing echo/prompt latency before the main-process IPC path while keeping unrelated background output on the normal batch timer.
...
- Core stream fan-out now walks the indexed subscription sets directly and keeps only socket deduplication, avoiding a per-event candidate Set allocation and redundant scope recheck on high-frequency `session.output`.
